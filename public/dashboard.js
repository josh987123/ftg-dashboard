...
const sectionOrder = [
  ...
  'cashReports', 'accounts', 'payments', 'apAging', 'receivablesPayables', 'jobOverview', 'jobBudgets', 'jobActuals', 'costCodes', 'missingBudgets', 'jobAnalytics'
];
...
/* ============================================================
   AP AGING REPORT MODULE
============================================================ */
let apAgingData = [];
let apAgingFiltered = [];
let apAgingCurrentPage = 1;
let apAgingPageSize = 25;
...
function loadApAgingData(retryCount = 0) {
  const loadingOverlay = document.getElementById('apAgingLoadingOverlay');
  if (loadingOverlay) loadingOverlay.classList.remove('hidden');
  ...
  fetch('/api/ap-aging')
    ...
    .then(data => {
      if (data.success) {
        apAgingData = data.vendors || [];
        apAgingFiltered = [...apAgingData];
    ...
    .catch(err => {
      ...
      if (retryCount < 2) {
        setTimeout(() => loadApAgingData(retryCount + 1), 500);
        return;
      }
      const tbody = document.getElementById('apAgingTableBody');
      if (tbody) {
        tbody.innerHTML = '<tr><td colspan="8" class="error-cell">Error loading AP aging data. <button onclick="loadApAgingData()" class="retry-btn">Retry</button></td></tr>';
      }
      if (loadingOverlay) loadingOverlay.classList.add('hidden');
    });
}
...
function updateApAgingSummary(totals) {
  ...
  setVal('apAgingTotalDue', totals.total_due);
  setVal('apAgingCurrent', totals.current);
  setVal('apAging31to60', totals.days_31_60);
  setVal('apAging61to90', totals.days_61_90);
  setVal('apAging90Plus', totals.days_90_plus);
  setVal('apAgingRetainage', totals.retainage);
}

function initApAgingEventHandlers() {
  // Search filter
  ...
  if (searchInput) {
    searchInput.addEventListener('input', debounce(() => {
      ...
      if (term) {
        apAgingFiltered = apAgingData.filter(v => 
          v.vendor_name.toLowerCase().includes(term)
        );
      } else {
        apAgingFiltered = [...apAgingData];
      }
      apAgingCurrentPage = 1;
      applyApAgingSort();
      renderApAgingTable();
    }, 300));
  }
  ...
  // Sortable columns
  document.querySelectorAll('#apAgingTable th.sortable').forEach(th => {
    th.addEventListener('click', () => {
      const col = th.dataset.sort;
      if (apAgingSortColumn === col) {
        apAgingSortDirection = apAgingSortDirection === 'asc' ? 'desc' : 'asc';
      } else {
        apAgingSortColumn = col;
        apAgingSortDirection = 'desc';
      }
      applyApAgingSort();
      renderApAgingTable();
  ...
  // Pagination
  const prevBtn = document.getElementById('apAgingPrevPage');
  const nextBtn = document.getElementById('apAgingNextPage');
  const pageSelect = document.getElementById('apAgingPageSize');
  
  if (prevBtn) prevBtn.addEventListener('click', () => {
    if (apAgingCurrentPage > 1) {
      apAgingCurrentPage--;
      renderApAgingTable();
    }
  });
  ...
  if (nextBtn) nextBtn.addEventListener('click', () => {
    const totalPages = Math.ceil(apAgingFiltered.length / apAgingPageSize);
    if (apAgingCurrentPage < totalPages) {
      apAgingCurrentPage++;
      renderApAgingTable();
    }
  });
  ...
  if (pageSelect) pageSelect.addEventListener('change', () => {
    apAgingPageSize = parseInt(pageSelect.value, 10);
    apAgingCurrentPage = 1;
    renderApAgingTable();
  });
}

function applyApAgingSort() {
  apAgingFiltered.sort((a, b) => {
    let aVal = a[apAgingSortColumn];
    let bVal = b[apAgingSortColumn];
    
    // Numeric columns
    if (['vendor_id', 'total_due', 'current', 'days_31_60', 'days_61_90', 'days_90_plus', 'retainage'].includes(apAgingSortColumn)) {
      aVal = parseFloat(aVal) || 0;
      bVal = parseFloat(bVal) || 0;
    } else {
      aVal = (aVal || '').toString().toLowerCase();
    ...
    if (aVal < bVal) return apAgingSortDirection === 'asc' ? -1 : 1;
    if (aVal > bVal) return apAgingSortDirection === 'asc' ? 1 : -1;
    return 0;
  });
}
...
function updateApAgingSortIndicators() {
  document.querySelectorAll('#apAgingTable th.sortable').forEach(th => {
    th.classList.remove('sort-asc', 'sort-desc');
    if (th.dataset.sort === apAgingSortColumn) {
      th.classList.add(apAgingSortDirection === 'asc' ? 'sort-asc' : 'sort-desc');
    }
  });
}

function renderApAgingTable() {
  const tbody = document.getElementById('apAgingTableBody');
  if (!tbody) return;
  
  const totalPages = Math.ceil(apAgingFiltered.length / apAgingPageSize) || 1;
  const startIdx = (apAgingCurrentPage - 1) * apAgingPageSize;
  const endIdx = startIdx + apAgingPageSize;
  const pageData = apAgingFiltered.slice(startIdx, endIdx);
  
  if (pageData.length === 0) {
    tbody.innerHTML = '<tr><td colspan="8" class="empty-cell">No vendors with outstanding balances</td></tr>';
  } else {
    tbody.innerHTML = pageData.map(v => \`
      <tr>
        <td>\${v.vendor_id}</td>
        <td>\${escapeHtml(v.vendor_name)}</td>
        <td class="number-col">\${formatCurrency(v.total_due)}</td>
        <td class="number-col">\${formatCurrency(v.current)}</td>
        <td class="number-col">\${formatCurrency(v.days_31_60)}</td>
        <td class="number-col">\${formatCurrency(v.days_61_90)}</td>
        <td class="number-col \${v.days_90_plus > 0 ? 'negative' : ''}">\${formatCurrency(v.days_90_plus)}</td>
        <td class="number-col">\${formatCurrency(v.retainage)}</td>
      </tr>
    \`).join('');
  }
  ...
  const pageTotals = {
    total_due: apAgingFiltered.reduce((s, v) => s + (v.total_due || 0), 0),
    current: apAgingFiltered.reduce((s, v) => s + (v.current || 0), 0),
    days_31_60: apAgingFiltered.reduce((s, v) => s + (v.days_31_60 || 0), 0),
    days_61_90: apAgingFiltered.reduce((s, v) => s + (v.days_61_90 || 0), 0),
    days_90_plus: apAgingFiltered.reduce((s, v) => s + (v.days_90_plus || 0), 0),
    retainage: apAgingFiltered.reduce((s, v) => s + (v.retainage || 0), 0)
  };
  
  const setFoot = (id, val) => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = '<strong>' + formatCurrency(val) + '</strong>';
  };
  setFoot('apAgingFootTotal', pageTotals.total_due);
  setFoot('apAgingFootCurrent', pageTotals.current);
  setFoot('apAgingFoot31to60', pageTotals.days_31_60);
  setFoot('apAgingFoot61to90', pageTotals.days_61_90);
  setFoot('apAgingFoot90Plus', pageTotals.days_90_plus);
  setFoot('apAgingFootRetainage', pageTotals.retainage);
  
  // Update pagination
  const pageInfo = document.getElementById('apAgingPageInfo');
  const prevBtn = document.getElementById('apAgingPrevPage');
  const nextBtn = document.getElementById('apAgingNextPage');
  const countEl = document.getElementById('apAgingCount');
  
  if (pageInfo) pageInfo.textContent = 'Page ' + apAgingCurrentPage + ' of ' + totalPages;
  if (prevBtn) prevBtn.disabled = apAgingCurrentPage <= 1;
  if (nextBtn) nextBtn.disabled = apAgingCurrentPage >= totalPages;
  if (countEl) countEl.textContent = '(' + apAgingFiltered.length + ' vendors)';
