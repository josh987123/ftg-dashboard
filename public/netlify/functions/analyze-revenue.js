...
exports.handler = async function(event, context) {
  ...
  try {
    const { revenueData, periodInfo } = JSON.parse(event.body);

    if (!revenueData) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Missing revenue data' })
      };
    }
    ...
    const systemPrompt = `You are a CFO analyzing a construction company's Revenue data.
...
- Focus on revenue trends, growth rates, seasonal patterns, and forecast implications
