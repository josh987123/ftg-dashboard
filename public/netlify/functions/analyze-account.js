...
exports.handler = async function(event, context) {
  ...
  try {
    const { accountData, accountInfo, periodInfo } = JSON.parse(event.body);

    if (!accountData) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Missing account data' })
      };
    }
    ...
    const systemPrompt = `You are a CFO analyzing a construction company's GL account activity.
...
- Focus on account trends, unusual activity, seasonal patterns, and implications
- DO NOT add any other fields or sections`;

    const userPrompt = `Analyze this GL account activity for FTG Builders:
