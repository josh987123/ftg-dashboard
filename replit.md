...
- **Job Overview**: High-level summary view with key metric tiles (Total Jobs, Avg % Complete, Avg Profit Margin) and three bar charts: Revenue (Contracted, Earned, Billed, Backlog), Over/(Under) Bill (Billed vs Earned with variance), and Costs (Revised Cost vs Actual Cost). Features same filtering options as other job views (status, PM, customer).
