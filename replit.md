...
- **Income Statement**: Supports single period and matrix views with AI analysis.
  - **Revenue to Net Income Waterfall Chart**: Displayed under AI Analysis section, only visible in single period mode. Shows waterfall flow at medium detail level: Revenue → Direct Expenses → Indirect Expenses → Gross Profit → (Operating expense categories: Salaries, Facility, Travel/Ent, Insurance, Prof Svcs, Admin/Other) → Operating Income → Other Inc/Exp → Net Profit. Uses floating bars for proper deduction visualization. Profit bars shown in green, deduction bars in red.
